 ...
 * Version corrigée pour résoudre le problème de duplication des boutons
 */
...
// Garde un état global pour déterminer si l'interface a déjà été initialisée
let exportImportInterfaceInitialized = false;
...
function afficherInterfaceExportImport() {
    // Si un modal existe déjà, le supprimer
    const existingModal = document.getElementById('modalExportImport');
    if (existingModal) {
...
// Initialisation - seulement créer le bouton s'il n'existe pas déjà
document.addEventListener('DOMContentLoaded', function() {
    // Éviter la duplication des événements et des boutons
    if (exportImportInterfaceInitialized) {
        return;
    }
    
    exportImportInterfaceInitialized = true;
    
    // Si c'est la page d'administration, configurer le bouton d'export/import
    if (window.location.pathname.includes('admin.html')) {
        // Trouver le bouton existant
        const existingButton = document.getElementById('btn-export-import');
        if (existingButton) {
            // Supprimer les anciens gestionnaires d'événement en clonant le bouton
        ...
        // Pour la page principale, ajouter le bouton uniquement s'il n'existe pas déjà
        if (!document.getElementById('btn-export-import') && estMJ()) {
            ...
            if (userInfo) {
                ...
                if (!existingButton) {
                    ...
                    // Insérer avant le bouton des paramètres
                    const btnSettings = document.getElementById('btn-settings');
                    if (btnSettings) {
                        userInfo.insertBefore(boutonExport, btnSettings);
                    } else {
                        userInfo.appendChild(boutonExport);
                    }
