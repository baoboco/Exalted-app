...
function getCheminIconeSVG(type) {
    ...
    // Si le type existe et a une icône définie
    if (typesSauvegardes[type] && typesSauvegardes[type].icone) {
        // Utiliser le chemin direct vers l'icône
        return `assets/icons/${typesSauvegardes[type].icone}.svg`;
    }
    
    // Sinon, essayer de deviner le chemin en utilisant le type comme nom de fichier
    if (type) {
        // Pour les icônes spéciales (autres icones)
        if (['bank', 'clock-tower', 'hospital', 'maze'].includes(type)) {
            return `assets/icons/autres icones/${type}.svg`;
        }
