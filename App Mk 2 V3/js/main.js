 ...
 * Version corrigée pour la gestion des niveaux
 */
...
function mettreAJourInterfaceRole() {
    if (CONFIG.utilisateur.role === "mj") {
        ...
        // Afficher les boutons réservés aux MJ
        document.querySelectorAll('.mj-only').forEach(elem => {
            elem.classList.remove('hidden');
            elem.style.display = '';
        });
        
        // Afficher le bouton d'admin s'il existe
        ...
        // Masquer les boutons réservés aux MJ
        document.querySelectorAll('.mj-only').forEach(elem => {
            elem.classList.add('hidden');
            elem.style.display = 'none';
        });
        
        // Masquer le bouton d'admin s'il existe
        const btnAdmin = document.getElementById('btn-admin');
        if (btnAdmin) {
...
function initCarte() {
    ...
    // Déterminer le chemin de l'image en fonction du niveau actuel
    let cheminImage;
    ...
    if (niveau === 0) {
        ...
        if (indexAbsolu < CONFIG.carte.cheminNiveaux.length) {
            ...
            // Si tout échoue, utiliser le niveau 0
            cheminImage = CONFIG.carte.cheminNiveaux[0];
            console.warn(`Niveau ${niveau} non trouvé, utilisation du niveau 0 par défaut`);
        }
    }
    ...
    if (CONFIG.debug.actif && CONFIG.debug.afficherCoordonees) {
        ...
        if (!document.getElementById('coords-debug')) {
            const debugElement = document.createElement('div');
            debugElement.id = 'coords-debug';
            ...
            document.getElementById('map-container').appendChild(debugElement);
        }
        
        map.on('mousemove', function(e) {
            const coord = e.latlng;
            const debugElement = document.getElementById('coords-debug');
            if (debugElement) {
                debugElement.textContent = `X: ${Math.round(coord.lng)}, Y: ${Math.round(coord.lat)}`;
            }
        });
    }
...
function chargerSousCartesPourFormulaire() {
    ...
    try {
        // D'abord vérifier dans CONFIG.sousCartes
        if (CONFIG.sousCartes) {
            // Ajouter chaque sous-carte comme option
            Object.entries(CONFIG.sousCartes).forEach(([id, sousCarte]) => {
        ...
        // Sinon, récupérer les sous-cartes sauvegardées
...
function chargerNiveauxPourFormulaire() {
    ...
    try {
        // Vérifier d'abord si les niveaux sont dans CONFIG
        if (CONFIG.carte.niveauxDisponibles && CONFIG.carte.niveauxDisponibles.length > 0) {
        ...
        if (niveauxSauvegardes) {
            ...
            const niveauxParDefaut = [
                {ordre: 0, nom: NOMS_NIVEAUX[0] || 'Surface'},
                {ordre: -1, nom: NOMS_NIVEAUX[1] || 'Souterrains'},
                {ordre: -2, nom: NOMS_NIVEAUX[2] || 'Égouts'},
                {ordre: -3, nom: NOMS_NIVEAUX[3] || 'Ruines anciennes'}
            ];
...
function configurerEvenements() {
    ...
    document.getElementById('btn-add-marker').addEventListener('click', function() {
        ...
        if (!estMJ() && !CONFIG.utilisateur.permissions.creerMarqueursOfficiels) {
            ...
            afficherAlerte("En tant que joueur, vous pouvez uniquement ajouter des marqueurs personnels.", "info");
        }
    ...
    document.getElementById('btn-help').addEventListener('click', function() {
        afficherAlerte(`Bienvenue sur la carte interactive de Nexus!\n\n` +
...
function ajouterNouveauMarqueur() {
    ...
    if (!positionMarqueurTemp) {
        afficherAlerte("Veuillez cliquer sur la carte pour choisir l'emplacement du marqueur.", "warning");
        return;
    }
    ...
    if (selectInteraction) {
        ...
        if (marqueurData.interaction.type === 'sous-carte') {
            ...
            if (selectSousCarte && selectSousCarte.value) {
                ...
                afficherAlerte("Veuillez sélectionner une sous-carte.", "warning");
                return;
            }
        }
    }
    ...
    if (nouveauMarqueur) {
        ...
        afficherAlerte(`Marqueur "${nom}" créé avec succès`, "success");
    } else {
        afficherAlerte("Erreur lors de la création du marqueur.", "danger");
    }
}
