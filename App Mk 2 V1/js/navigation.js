    ...
    if (!document.getElementById('niveau-selecteur')) {
        ...
        niveauxAAfficher.forEach(niveau => {
            ...
            boutonNiveau.addEventListener('click', function() {
                ...
                // Vérifier ensuite dans la configuration par défaut
                if (niveauIndex >= 0 && niveauIndex < CONFIG.carte.cheminNiveaux.length) {
                    changerNiveau(niveauIndex);
                } else {
                    // L'index du niveau n'est pas disponible
                    console.warn(`Niveau ${niveauIndex} (${nomNiveau}) non disponible dans la configuration`);
                    afficherAlerte(`Le niveau ${nomNiveau} n'est pas encore disponible`, "warning");
                }
            });
...
function changerNiveau(niveau) {
    ...
    // Vérifier que le niveau demandé existe et est disponible
    if (niveau < 0 || niveau >= CONFIG.carte.cheminNiveaux.length) {
        console.warn(`Niveau inexistant: ${niveau}`);
        afficherAlerte(`Le niveau ${niveau} n'est pas disponible`, "warning");
        return false;
    }
...
function changerNiveauAdmin(niveau, niveauxPersonnalises) {
    ...
    if (!niveauObj || !niveauObj.fichier) {
        ...
        afficherAlerte(`Le niveau ne peut pas être chargé (fichier manquant)`, "warning");
        return false;
    }
    ...
    try {
        ...
        const cheminComplet = `cartes/${niveauObj.fichier}`;
...
function retourCarteNiveau() {
    ...
    try {
        ...
        if (!cheminImage) {
            throw new Error(`Chemin d'image non défini pour le niveau ${etatPrecedent.niveau}`);
        }
