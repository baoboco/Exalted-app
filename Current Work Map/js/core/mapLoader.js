...
class MapLoader {
  ...
  async loadSvgLayer(layerId, svgUrl) {
    return new Promise((resolve, reject) => {
      ...
      // Créer une nouvelle couche d'image SVG
      try {
        const svgLayer = L.svgOverlay(svgUrl, [
          [0, 0],
          [2000, 2000]
        ], {
          interactive: true,
  ...
  activateLayer(layerId) {
    ...
    if (this.currentLayerId && this.svgLayers[this.currentLayerId]) {
      ...
      this.svgLayers[this.currentLayerId].getElement().classList.remove('active-layer');
    }
    
    // Activer la nouvelle couche
    this.svgLayers[layerId].setOpacity(1);
    this.svgLayers[layerId].getElement().classList.add('active-layer');
  ...
  extractSvgElements(svgElement, layerId) {
    ...
    // Extraire uniquement les éléments qui nous intéressent, plutôt que tous les éléments avec un ID
  ...
  processInvisibleElements(invisibleElements, quarterGroups, elements, layerId, svgElement) {
    ...
    invisibleElements.forEach(element => {
      ...
      const quartierName = element.id.substring(4); // Sans "inv_"
